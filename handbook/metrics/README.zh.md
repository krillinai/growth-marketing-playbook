...
| KPI（关键绩效指标） | 为特定目标与负责人选出的指标 | KPI 地位取决于决策，而不是名称。 |
| 目标 | 一段时间内指标期望值或区间 | 目标不能证明数值可实现或健康。 |
| Benchmark | 来自另一时期、分群或公司的参考分布 | 迁移需要可比较定义、环境与成熟度。 |
| 维度 | 市场、来源、设备、角色或 Cohort（同期群）等拆分 | 过多拆分会制造噪音与 False Discovery。 |
| 模型输出 | LTV（客户生命周期价值）、风险、Propensity 或归因功劳等预测 | 需要校准与不确定性，并与已观察结果分开。 |
...
- **结果指标要拆成领先指标。** LTV、收入、GMV（成交总额）太滞后，需要拆到流量、转化、留存、复购和实验动作。
- **Agent 输出必须连接指标。** 每个分析、内容、CRO（转化率优化）、实验或留存建议，都应该说明影响哪个指标。
...
Cohort（同期群）是拥有共同起始时间、曝光、行为或产品状态，并按照相同经过时长观察的一组实体。“7 月注册 Cohort”不同于“7 月全部活跃用户”：前者从共同起点追踪固定群体，后者混合了不同年龄的客户。

| 结构 | 分组依据 | 最适合回答的问题 |
| --- | --- | --- |
| Cohort | 共同起点、曝光、行为或状态 | 相同成熟度下的结果是否变化？ |
| Segment | 相对稳定的客户或账户属性 | 是否需要不同产品、信息、服务或市场路径？ |
| Funnel | 有顺序的价值状态或转化 | 损失发生在哪里，主要约束是什么？ |

Cohort 可以按照决策相关 Segment 拆分，Funnel 也可以跨 Cohort 比较，但三者回答不同问题，不能互相替代。

### 构建 Cohort 分析

1. 明确分析需要支持的决策与结果。
2. 冻结实体、资格、起始事件、合格事件、身份规则、周期、时区与观察截止时间。
3. 把每个实体分配到一个原始 Cohort，并从共同起点计算结果年龄。
4. 标记哪些单元格已经完整成熟；保留未完整成熟或右删失单元格，不能把它们当成失败。
5. 先跨起始周期比较相同 Cohort Age，再按照能够改变决策的维度拆分。
6. 同时报告分子、分母、比率与绝对贡献。
7. 提出机制前，先把总体变化拆成 Cohort Mix 变化与 Cohort 内部变化。

下面是示意性的每周留存表：

| 注册 Cohort | 合格用户 | 第 1 周 | 第 2 周 | 第 4 周 |
| --- | ---: | ---: | ---: | ---: |
| 6 月 1 日 | 100 | 52% | 39% | 31% |
| 6 月 8 日 | 120 | 57% | 42% | 34% |
| 6 月 29 日 | 140 | 59% | 尚未成熟 | 尚未成熟 |

每一列代表相同经过时长，而不是同一自然周。6 月 29 日 Cohort 只能用于第 1 周判断；把尚未完整观察的后续周期与早期成熟 Cohort 比较，会制造虚假的下降。

```text
Cohort Age t 的结果率
= Age t 达到结果定义的原始 Cohort 合格实体
/ 已经完整成熟至 Age t 的原始 Cohort 合格实体
```

### 解读规则

- 只比较兼容的实体、起始事件、结果定义、年龄、市场与产品版本。
- 在 Segment 切分旁保留统一总览，让 Mix 变化与绝对贡献保持可见。
- 把趋势或相关性视为诊断信号，不能直接证明产品变化、渠道或某项行为造成结果。
- 把小分母、事件缺失、身份变化、迟到数据、季节性、实验曝光与幸存者偏差保留为局限。
- 尽可能预先声明与决策相关的切分，不能搜索数百个切片直到出现有利结果。

重复价值曲线、流失、召回与增长核算参见[留存](../retention/README.zh.md)。使用 [Cohort Analysis Skill](https://github.com/krillinai/growth-skills/tree/main/skills/cohort-analysis) 可以把用户提供的事件或汇总数据转化为有明确边界的 Cohort 分析。
...
使用自然频率上的价值行为、Cohort 留存、参与分布、召回、信任与商业化。只有活动代表价值时，DAU（日活跃用户）或 MAU（月活跃用户）才有意义。
...
| Growth Loop | Growth Loop | 增长飞轮/增长闭环 | 设计用户行为带来新用户的机制 | 分享、UGC（用户生成内容）、内容和产品循环 |
...
| --- | --- | --- | --- | --- |
| SEO | Search Engine Optimization | 搜索引擎优化 | 提升自然搜索排名和自然流量 | SEO / GEO（生成式引擎优化）模块 |
...
- **激活：** Activation Rate、TTV、关键行为完成率、email/SMS（短信）订阅、加购。
...
- [客户分群与 ICP（理想客户画像）](../customer-segmentation/README.zh.md)：只有分群证据能够改变决策时才使用它。
